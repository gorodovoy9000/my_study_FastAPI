...
async def get_hotel(hotel_id: IdDep) -> HotelsSchema:
    async with async_session_maker() as session:
...
async def create_hotel(schema_create: HotelsWriteSchema):
    async with async_session_maker() as session:
...
@only_one_error_handler
async def update_hotel(hotel_id: IdDep, schema_update: HotelsWriteSchema):
    async with async_session_maker() as session:
...
@only_one_error_handler
async def partial_update_hotel(hotel_id: IdDep, schema_patch: HotelsPatchSchema):
    async with async_session_maker() as session:
